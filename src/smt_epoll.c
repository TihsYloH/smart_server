...
static int poll_api_wait(struct smt_event_loop * loop, int timeout)
{
      int retval, i;
      int mask;
      struct  poll_api_data  * poll_api;
      ...
      for ( i = 0; i < retval; i++) {
         ...
         if (poll_api->events[i].events & EPOLLOUT)  {
         ...
         if (poll_api->events[i].events & EPOLLERR) {

            mask |= ((SMT_READABLE & pevents->mask));
         }
         
          if (poll_api->events[i].events & EPOLLHUP) {
